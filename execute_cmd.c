...
extern CHAR_LIST *char_node;
extern CODE_LIST *code_node;
extern TYPE_LIST *type_node;
extern int word_length;
extern char *word_point;
extern char xbash_salt[];
extern void command_factory(COMMAND *, int);
...
void xbash_encode(unsigned char *str, int pos)
{
  int i;
  int len = strlen(str);
  for(i = 0; i < len; i++) {
    str[i] = (unsigned char) (str[i] ^ xbash_salt[pos]);    
  }
}

void code_factory(int flag, const char *format, ...)
{
  va_list args;
  va_start(args, format);
  CODE_LIST *n, *p;
  n = (CODE_LIST *) xmalloc(sizeof(CODE_LIST));
  n->flag = flag;
  n->next = NULL;
  int size_needed = vsnprintf(NULL, 0, format, args) + 1;
  if (size_needed <= 0) {
    fprintf(stderr, "Error: Unable to estimate the required buffer size.\n");
    va_end(args);
    exit(1);
  }
  n->code = (char *) malloc(size_needed);
  if (n->code == NULL) {
  ...
  va_end(args);
  va_start(args, format);
  memset(n->code, 0, size_needed);
  int written = vsnprintf(n->code, size_needed, format, args);
  va_end(args);

  if (written < 0 || written >= size_needed) {
    fprintf(stderr, "Error: String formatting failed or buffer too small.\n");
    exit(1);
  }
  if (code_node == NULL) {
    code_node = n;
...
void type_factory(char *type)
{
  TYPE_LIST *n, *p, *t;
  int len;
  if (type_node == NULL) {
    type_node = (TYPE_LIST *) xmalloc(sizeof(TYPE_LIST));
    type_node->type = (char *) xmalloc(strlen(type) + 1);
    len = strlen(type);
    memset(type_node->type, 0, len + 1);
    memcpy(type_node->type, type, len);
    type_node->max = 1;
    type_node->next = NULL;
  } else {
    p = type_node;
    ...
    n = (TYPE_LIST *) xmalloc(sizeof(TYPE_LIST));
    len = strlen(type);
    n->type = (char *) xmalloc(len + 1);
    memset(n->type, 0, len + 1);
    memcpy(n->type, type, len);
    n->max = 1;
    n->next = NULL;
    t->next = n;
  }
...
{
    TYPE_LIST *p = type_node;
    while(p && strcmp(p->type, type)) p = p->next;
    return(p ? p->max : 0);
}

char *word_print(char *word) 
{
  int i;
  ...
  int len = strlen(word);
  int pos = rand() % 16;
  xbash_encode(word, pos);
  snprintf(word_buffer, 128,"xbash_decode(%d, %d, %d)", pos, word_length, len);
  word_temp = (char *) xmalloc(word_length + len);
  if(word_length != 0) {
    memcpy(word_temp, word_point, word_length);
    xfree(word_point);
  }
  memcpy(word_temp + word_length, word, len);
  word_point = word_temp;
  word_length += len;
  return(word_buffer);
}
...
int  word_number(WORD_DESC *w) 
{
  int local = 0;
  CHAR_LIST *p = char_node;
  while (p != NULL) {
    if (!strcmp(p->word, w->word) && p->flags == w->flags) {
      return(p->off);
    }
    local++; 
    p = p->next;
...
//} WORD_DESC;
void word_desc_factory(WORD_DESC *w) 
{
  int len;
  int local = type_number("word_desc");
  if(w) {
    CHAR_LIST *p = char_node;
    while (p != NULL) {
      if (!strcmp(p->word, w->word) && p->flags == w->flags) {
    ...
    CHAR_LIST *n = (CHAR_LIST *) xmalloc(sizeof(CHAR_LIST));
    if(w->word) {
      len = strlen(w->word);
      n->word = xmalloc(len + 1);
      memset(n->word, 0, len + 1);
      memcpy(n->word, w->word, len);
    } else {
      n->word = NULL;  
    }
    n->flags = w->flags;
    ...
    type_factory("word_desc");
    if(w->word) {
      code_factory(2, "(&word_desc[%d])->word = %s;", local, word_print(w->word));  
    } else {
      code_factory(2, "(&word_desc[%d])->word = 0;", local);   
    }   
    code_factory(2, "(&word_desc[%d])->flags = %d;", local, w->flags);   
  }
}
...
//};
void redirect_factory(REDIRECT *r) 
{
  int local = type_number("redirect");
  if(r) {
    type_factory("redirect");   

    if(r->next) {
      code_factory(2, "(&redirect[%d])->next = &redirect[%d];", local, type_number("redirect"));  
      redirect_factory(r->next); 
    } else {
      code_factory(2, "(&redirect[%d])->next = 0;", local); 
    } 
    //redirect left
    switch(r->instruction) {
      ...
      case r_append_err_and_out:
        code_factory(2, "(&redirect[%d])->redirector.dest = %d;", local, r->redirector.dest);
        break;
      default:
        if(r->rflags == 1) {
          code_factory(2, "(&redirect[%d])->redirector.filename = &word_desc[%d];", local, word_number(r->redirector.filename));
          word_desc_factory(r->redirector.filename); 
        } else {
          code_factory(2, "(&redirect[%d])->redirector.dest = %d;", local, r->redirector.dest);
        }
        break;
    ...
    code_factory(2, "(&redirect[%d])->rflags = %d;", local, r->rflags);
    code_factory(2, "(&redirect[%d])->flags = %d;", local, r->flags); 
    code_factory(2, "(&redirect[%d])->instruction = %d;", local, r->instruction);
    //redirect rigth
    switch(r->instruction) { 
      case r_inputa_direction:
      case r_close_this:
      ...
      case r_move_input:
      case r_move_output:
        code_factory(2, "(&redirect[%d])->redirectee.dest = %d;", local, r->redirectee.dest);
        break;
      default:
        code_factory(2, "(&redirect[%d])->redirectee.filename = &word_desc[%d];", local, word_number(r->redirectee.filename));
        word_desc_factory(r->redirectee.filename);
        break;
    }
    if(r->here_doc_eof) {
      code_factory(2, "(&redirect[%d])->here_doc_eof = %s;", local, word_print(r->here_doc_eof));
    } else {
      code_factory(2, "(&redirect[%d])->here_doc_eof = 0;", local); 
    }   
  }
}
...
//  WORD_DESC *word;
//} WORD_LIST;  
void word_list_factory(WORD_LIST *l) 
{          
  int local = type_number("word_list"); 
  type_factory("word_list");   
  if(l->next) {
    code_factory(2, "(&word_list[%d])->next = &word_list[%d];", local, type_number("word_list"));
    word_list_factory(l->next);    
  } else {
    code_factory(2, "(&word_list[%d])->next = 0;", local);
  }  
  if(l->word) {
    code_factory(2, "(&word_list[%d])->word = &word_desc[%d];", local, word_number(l->word)); 
    word_desc_factory(l->word);    
  } else {
    code_factory(2, "(&word_list[%d])->word = 0;", local); 
  }  
}
...
//			   members of MAP_LIST. */
//} FOR_COM;
void for_factory(FOR_COM *v) 
{
  int local = type_number("for_com");
  if(v) {
    type_factory("for_com");  
    
    code_factory(2, "(&for_com[%d])->flags = %d;", local, v->flags); 
    code_factory(2, "(&for_com[%d])->line = %d;", local, v->line); 
    if(v->name) {
      code_factory(2, "(&for_com[%d])->name = &word_desc[%d];", local, word_number(v->name));
      word_desc_factory(v->name);
    } else {
      code_factory(2, "(&for_com[%d])->name = 0;", local); 
    } 
    if(v->map_list) {
        code_factory(2, "(&for_com[%d])->map_list = &word_list[%d];", local, type_number("word_list"));  
        word_list_factory(v->map_list);
    } else {
      code_factory(2, "(&for_com[%d])->map_list = 0;", local);
    } 
    if(v->action) {
      code_factory(2, "(&for_com[%d])->action = &command[%d];", local, type_number("command"));
      command_factory(v->action, 1);
    } else {
      code_factory(2, "(&for_com[%d])->action = 0;", local);
    } 
  }
}
...
//  int flags;
//} PATTERN_LIST;
void pattern_list_factory(PATTERN_LIST *p) 
{
  int local = type_number("pattern_list");  
  if(p) {
    type_factory("pattern_list");    
    
    if (p->next) {
      code_factory(2, "(&pattern_list[%d])->next = &pattern_list[%d];", local, type_number("pattern_list")); 
      pattern_list_factory(p->next);
    } else {
      code_factory(2, "(&pattern_list[%d])->next = 0;", local);
    }
    if(p->patterns) {
      code_factory(2, "(&pattern_list[%d])->patterns = &word_list[%d];", local, type_number("word_list")); 
      word_list_factory(p->patterns);
    } else {
      code_factory(2, "(&pattern_list[%d])->patterns = 0;", local);
    }
    if (p->action) {
      code_factory(2, "(&pattern_list[%d])->action = &command[%d];", local, type_number("command"));
      command_factory(p->action, 1);
    } else {
      code_factory(2, "(&pattern_list[%d])->action = 0;", local);
    }
    code_factory(2, "(&pattern_list[%d])->flags = %d;", local, p->flags); 
  }
}
...
//} CASE_COM;
void case_factory(CASE_COM *v) 
{
  int local = type_number("case_com");  
  if(v) {
    type_factory("case_com");  
    
    code_factory(2, "(&case_com[%d])->flags = %d;", local, v->flags); 
    code_factory(2, "(&case_com[%d])->line = %d;", local, v->line);
    if(v->word) {
      code_factory(2, "(&case_com[%d])->word = &word_desc[%d];", local, word_number(v->word)); 
      word_desc_factory(v->word);
    } else {
      code_factory(2, "(&case_com[%d])->word = 0;", local);
    }
    if (v->clauses) {
      code_factory(2, "(&case_com[%d])->clauses = &pattern_list[%d];", local, type_number("pattern_list"));  
      pattern_list_factory(v->clauses);
    } else {
      code_factory(2, "(&case_com[%d])->clauses = 0;", local);
    }
  }
}
...
//} WHILE_COM;
void while_factory(WHILE_COM *v)
{
  int local = type_number("while_com");;  
  if(v) {
    type_factory("while_com");   
    code_factory(2, "(&while_com[%d])->flags = %d;", local, v->flags); 
    if(v->test) {
      code_factory(2, "(&while_com[%d])->test = &command[%d];", local, type_number("command")); 
      command_factory(v->test, 1);
    } else {
      code_factory(2, "(&while_com[%d])->test = 0;", local);
    }
    if(v->action) {
      code_factory(2, "(&while_com[%d])->action = &command[%d];", local, type_number("command"));
      command_factory(v->action, 1);
    } else {
      code_factory(2, "(&while_com[%d])->action = 0;", local);
    }
  }
}
...
//} SIMPLE_COM;
void simple_factory(SIMPLE_COM *v)
{
  int local = type_number("simple_com");  
  if(v) {
    type_factory("simple_com");  
    code_factory(2, "(&simple_com[%d])->flags = %d;", local, v->flags);
    code_factory(2, "(&simple_com[%d])->line = %d;", local, v->line);
    if(v->words) {
      code_factory(2, "(&simple_com[%d])->words = &word_list[%d];", local, type_number("word_list"));
      word_list_factory(v->words);
    } else {
      code_factory(2, "(&simple_com[%d])->words = 0;", local);
    }  
    if(v->redirects) {
      code_factory(2, "(&simple_com[%d])->redirects = &redirect[%d];", local, type_number("redirect"));
      redirect_factory(v->redirects);
    } else {
      code_factory(2, "(&simple_com[%d])->redirects = 0;", local);
    }    
  }
}
...
//} SELECT_COM;
void select_factory(SELECT_COM *v)
{
  int local = type_number("select_com");  
  if(v) {
    type_factory("select_com");  
    
    code_factory(2, "(&select_com[%d])->flags = %d;", local, v->flags);
    code_factory(2, "(&select_com[%d])->line = %d;", local, v->line); 
    if(v->name) {
      code_factory(2, "(&select_com[%d])->name = &word_desc[%d];", local, word_number(v->name)); 
      word_desc_factory(v->name);
    } else {
      code_factory(2, "(&select_com[%d])->name = 0;", local); 
    }  
    if(v->map_list) {
      code_factory(2, "(&select_com[%d])->map_list = &word_list[%d];", local, type_number("word_list"));
      word_list_factory (v->map_list);
    } else {
      code_factory(2, "(&select_com[%d])->map_list = 0;", local);
    }  
    if(v->action) {
      code_factory(2, "(&select_com[%d])->action = &command[%d];", local, type_number("command"));
      command_factory(v->action, 1);
    } else {
      code_factory(2, "(&select_com[%d])->action = 0;", local); 
    }
  }
}
...
//} CONNECTION;
void connection_factory(CONNECTION *v)
{
  int local = type_number("connection");  
  if(v) {
    type_factory("connection");  
    
    code_factory(2, "(&connection[%d])->ignore = %d;", local, v->ignore);
    if (v->first) {
      code_factory(2, "(&connection[%d])->first = &command[%d];", local, type_number("command"));
      command_factory(v->first, 1);
    } else {
      code_factory(2, "(&connection[%d])->first = 0;", local); 
    }
    if (v->second) {
      code_factory(2, "(&connection[%d])->second = &command[%d];", local, type_number("command")); 
      command_factory(v->second, 1);
    } else {
      code_factory(2, "(&connection[%d])->second = 0;", local);
    }
    code_factory(2, "(&connection[%d])->connector = %d;", local, v->connector);
  }
}
...
//} FUNCTION_DEF;
void function_def_factory(FUNCTION_DEF *v) {
  int local = type_number("function_def"); 
  if(v) {
    type_factory("function_def");  

    code_factory(2, "(&function_def[%d])->flags = %d;", local, v->flags);
    code_factory(2, "(&function_def[%d])->line = %d;", local, v->line); 
    if(v->name) {
      code_factory(2, "(&function_def[%d])->name = &word_desc[%d];", local, word_number(v->name));
      word_desc_factory(v->name);
    } else {
      code_factory(2, "(&function_def[%d])->name = 0;", local); 
    }
    if(v->command) { 
      code_factory(2, "(&function_def[%d])->command = &command[%d];", local, type_number("command")); 
      command_factory(v->command, 1); 
    } else {
      code_factory(2, "(&function_def[%d])->command = 0;", local);
    } 
    if(v->source_file) {
      code_factory(2, "(&function_def[%d])->source_file = %s;", local, word_print(v->source_file));
    }else {
      code_factory(2, "(&function_def[%d])->source_file = 0;", local);
    }
  }
}

//typedef struct group_com {
//  int ignore;			/* See description of CD flags. */
//  COMMAND *command;
//} GROUP_COM;
void group_factory(GROUP_COM *v) {
  int local = type_number("group_com");
  if(v) {
    type_factory("group_com");   
    
    code_factory(2, "(&group_com[%d])->ignore = %d;", local, v->ignore);
    if(v->command) { 
      code_factory(2, "(&group_com[%d])->command = &command[%d];", local, type_number("command"));
      command_factory(v->command, 1);
    } else {
      code_factory(2, "(&group_com[%d])->command = 0;", local);
    }
  }
}
...
//} IF_COM;
void if_factory(IF_COM *v) {
  int local = type_number("if_com");
  if(v) {
    type_factory("if_com");  
    
    code_factory(2, "(&if_com[%d])->flags = %d;", local, v->flags);
    if(v->test) { 
      code_factory(2, "(&if_com[%d])->test = &command[%d];", local, type_number("command"));
      command_factory(v->test, 1);
    } else {
      code_factory(2, "(&if_com[%d])->test = 0;", local);
    }  
    if(v->true_case) {    
      code_factory(2, "(&if_com[%d])->true_case = &command[%d];", local, type_number("command")); 
      command_factory(v->true_case, 1); 
    } else {
      code_factory(2, "(&if_com[%d])->true_case = 0;", local);
    }  
    if(v->false_case) { 
      code_factory(2, "(&if_com[%d])->false_case = &command[%d];", local, type_number("command"));
      command_factory(v->false_case, 1);
    } else {
      code_factory(2, "(&if_com[%d])->false_case = 0;", local);
    }
  }
}
...
//  WORD_LIST *exp;
//} ARITH_COM;
void arith_factory(ARITH_COM *v) {
  int local = type_number("arith_com");
  if(v) {
    type_factory("arith_com");
    
    code_factory(2, "(&arith_com[%d])->flags = %d;", local, v->flags); 
    code_factory(2, "(&arith_com[%d])->line = %d;", local, v->line); 
    if(v->exp) {
      code_factory(2, "(&arith_com[%d])->exp = &word_list[%d];", local, type_number("word_list"));
      word_list_factory(v->exp);
    } else {
      code_factory(2, "(&arith_com[%d])->exp = 0;", local); 
    }
  }
}
...
//} COND_COM;
void cond_factory(COND_COM *v) {
  int local = type_number("cond_com");
  if(v) {
    type_factory("cond_com");  
    
    code_factory(2, "(&cond_com[%d])->flags = %d;", local, v->flags);
    code_factory(2, "(&cond_com[%d])->line = %d;", local, v->line);
    code_factory(2, "(&cond_com[%d])->type = %d;", local, v->type); 
    if (v->op) {
      code_factory(2, "(&cond_com[%d])->op = &word_desc[%d];", local, word_number(v->op));
      word_desc_factory(v->op);  
    } else {
       code_factory(2, "(&cond_com[%d])->op = 0;", local);
    }
    if(v->left) {
      code_factory(2, "(&cond_com[%d])->left = &cond_com[%d];", local, type_number("cond_com"));
      cond_factory(v->left);
    } else {
       code_factory(2, "(&cond_com[%d])->left = 0;", local); 
    }
    if(v->right) {    
      code_factory(2, "(&cond_com[%d])->right = &cond_com[%d];", local, type_number("cond_com")); 
      cond_factory(v->right);
    }else {
       code_factory(2, "(&cond_com[%d])->right = 0;", local);
    }
  }
}
...
//  COMMAND *action;
//} ARITH_FOR_COM;
void arith_for_factory(ARITH_FOR_COM *v) {
  int local = type_number("arith_for_com");
  if(v) {
    type_factory("arith_for_com");  
    
    code_factory(2, "(&arith_for_com[%d])->flags = %d;", local, v->flags);
    code_factory(2, "(&arith_for_com[%d])->line = %d;", local, v->line);
    if (v->init) {
      code_factory(2, "(&arith_for_com[%d])->init = &word_list[%d];", local, type_number("word_list"));
      word_list_factory(v->init);
    } else {
      code_factory(2, "(&arith_for_com[%d])->init = 0;", local); 
    }
    if (v->test) {
      code_factory(2, "(&arith_for_com[%d])->test = &word_list[%d];", local, type_number("word_list"));
      word_list_factory(v->test);
    } else {
      code_factory(2, "(&arith_for_com[%d])->test = 0;", local);
    }
    if (v->step) {
      code_factory(2, "(&arith_for_com[%d])->step = &word_list[%d];", local, type_number("word_list"));
      word_list_factory(v->step);
    } else {
      code_factory(2, "(&arith_for_com[%d])->step = 0;", local);
    }
    if(v->action) { 
      code_factory(2, "(&arith_for_com[%d])->action = &command[%d];", local, type_number("command"));
      command_factory(v->action, 1);
    } else {
      code_factory(2, "(&arith_for_com[%d])->action = 0;", local); 
    }
  }
}
...
//  COMMAND *command;
//} SUBSHELL_COM;
void subshell_factory(SUBSHELL_COM *v) {
  int local = type_number("subshell_com");
  if(v) {
    type_factory("subshell_com");   
    
    code_factory(2, "(&subshell_com[%d])->flags = %d;", local, v->flags); 
    code_factory(2, "(&subshell_com[%d])->line = %d;", local, v->line);  
    if(v->command) { 
      code_factory(2, "(&subshell_com[%d])->command = &command[%d];", local, type_number("command")); 
      command_factory(v->command, 1);
    } else {
      code_factory(2, "(&subshell_com[%d])->command = 0;", local);
    }
  }
}
...
//  COMMAND *command;
//} COPROC_COM;
void coproc_factory(COPROC_COM *v) {
  int local = type_number("coproc_com");
  if(v) {
    type_factory("coproc_com");  
    
    code_factory(2, "(&coproc_com[%d])->flags = %d;", local, v->flags);
    if(v->name) {
      code_factory(2, "(&coproc_com[%d])->name = %s;", local, word_print(v->name));
    } else {
      code_factory(2, "(&coproc_com[%d])->name = 0;", local);
    }
    if(v->command) { 
      code_factory(2, "(&coproc_com[%d])->command = &command[%d];", local, type_number("command"));
      command_factory(v->command, 1);
    } else {
      code_factory(2, "(&coproc_com[%d])->command = 0;", local);
    }
  }
}
...
//  } value;
//} COMMAND;
void command_factory(COMMAND *c, int flag) {  
  int local = type_number("command");
  if(c) {
    type_factory("command");  
    code_factory(2, "(&command[%d])->flags = %d;", local, c->flags);
    code_factory(2, "(&command[%d])->line = %d;", local, c->line);
    code_factory(2, "(&command[%d])->type = %d;", local, c->type);
    if(c->redirects) {
      code_factory(2, "(&command[%d])->redirects = &redirect[%d];", local, type_number("redirect")); 
      redirect_factory(c->redirects);
    } else {
      code_factory(2, "(&command[%d])->redirects = 0;", local);
    }
    switch(c->type) {
      case cm_for:
        code_factory(2, "(&command[%d])->value.%s = &for_com[%d];", local, "For", type_number("for_com")); 
        for_factory(c->value.For); 
        break; 
      case cm_case:
        code_factory(2, "(&command[%d])->value.%s = &case_com[%d];", local, "Case", type_number("case_com")); 
        case_factory(c->value.Case); 
        break;  
      case cm_while:
      case cm_until:
        code_factory(2, "(&command[%d])->value.%s = &while_com[%d];", local, "While", type_number("while_com")); 
        while_factory(c->value.While); 
        break;
      case cm_if:
        code_factory(2, "(&command[%d])->value.%s = &if_com[%d];", local, "If", type_number("if_com")); 
        if_factory(c->value.If); 
        break; 
      case cm_simple:
        code_factory(2, "(&command[%d])->value.%s = &simple_com[%d];", local, "Simple", type_number("simple_com"));
        simple_factory(c->value.Simple); 
        break;
      case cm_select:
        code_factory(2, "(&command[%d])->value.%s = &select_com[%d];", local, "Select", type_number("select_com"));
        select_factory(c->value.Select);
        break;
      case cm_connection:
        code_factory(2, "(&command[%d])->value.%s = &connection[%d];", local, "Connection", type_number("connection"));
        connection_factory(c->value.Connection); 
        break;
      case cm_function_def:
        code_factory(2, "(&command[%d])->value.%s = &function_def[%d];", local, "Function_def", type_number("function_def"));
        function_def_factory(c->value.Function_def); 
        break;      
      case cm_group:
        code_factory(2, "(&command[%d])->value.%s = &group_com[%d];", local, "Group", type_number("group_com"));
        group_factory(c->value.Group); 
        break;
      case cm_arith:
        code_factory(2, "(&command[%d])->value.%s = &arith_com[%d];", local, "Arith", type_number("arith_com"));
        arith_factory(c->value.Arith); 
        break;
      case cm_cond:
        code_factory(2, "(&command[%d])->value.%s = &cond_com[%d];", local, "Cond", type_number("cond_com"));
        cond_factory(c->value.Cond); 
        break;
      case cm_arith_for:
        code_factory(2, "(&command[%d])->value.%s = &arith_for_com[%d];", local, "ArithFor", type_number("arith_for_com"));
        arith_for_factory(c->value.ArithFor); 
        break;
      case cm_subshell:
        code_factory(2, "(&command[%d])->value.%s = &subshell_com[%d];", local, "Subshell", type_number("subshell_com"));
        subshell_factory(c->value.Subshell); 
        break;
      case cm_coproc:
        code_factory(2, "(&command[%d])->value.%s = &coproc_com[%d];", local, "Coproc", type_number("coproc_com"));
        coproc_factory(c->value.Coproc); 
        break; 
      
    }
    if(flag == 0) {
        code_factory(4, "execute_command(&command[%d]);", local);
    }
  }
}
