...
static const char *main_code[] = {
  "/* Make by xbash",
  "   Function: Convert bash into C code",
  "   Author: ChenZhongChao",
  "   Birthdate: 2024-12-01",
  "   Github: https://github.com/chenzhch/xbash.git",
  "*/",
  " ",
  "/* shell.c -- GNU's idea of the POSIX shell specification. */",
  "",
  ...
  "extern char xbash_word[];",       
  "char *xbash_decode(int pos, int start, int len) ", 
  "{", 
  "  int i;", 
  "  char *result = xmalloc((size_t)(len + 1));", 
  "  memset(result, 0, len + 1);",
  "  for(i = 0; i < len; i++) {",
  "    sprintf(&result[strlen(result)], \"%c\", xbash_word[start + i] ^ xbash_salt[pos]);", 
  "  }",
  "  return(result);",
  "}",
  0
...
int copyright()
{
  printf("\033[1;34m\\\\  //  ||  ))     A     ((SSSS ||   ||\033[0m\n");
  printf("\033[1;34m \\\\//   || //     /A\\     \\\\    ||   ||\033[0m\n");
  printf("\033[1;34m  XX    ||BB     // \\\\      ))  |HHHHH|\033[0m\n");
  printf("\033[1;34m //\\\\   || \\\\   //AAA\\\\     //  ||   ||\033[0m\n");
  printf("\033[1;34m//  \\\\  ||  )) //     \\\\ SSSS)) ||   ||\033[0m\n");
  printf("\033[4;32mhttps://github.com/chenzhch/xbash.git\033[0m\n\n");
  return(0);
}

extern int gnu_error_format;
extern void char_free();
extern void type_free();
...
CHAR_LIST *char_node = NULL;
CODE_LIST *code_node = NULL;
TYPE_LIST *type_node = NULL;
unsigned char xbash_salt[16];
int word_length = 0;
char *word_point = NULL;
...
#endif /* !NO_MAIN_ENV_ARG */
{
 ...
 read_and_execute:
  ...
  i = 0;
  while(main_code[i]) fprintf(fp, "%s\n", main_code[i++]);
  fprintf(fp, "char xbash_salt[] = {");
  for(i = 0; i < 16; i++) {
    fprintf(fp, "0x%02X", (unsigned char) xbash_salt[i]);
    if(i != 15) {
      fprintf(fp, ", ");
    }
  }  
  fprintf(fp, "};\n"); 
  fprintf(fp, "char xbash_word[] = {\n");
  fprintf(fp, " ");
  for(i = 0; i < word_length - 1;) {
    fprintf(fp, " 0x%02X,", (unsigned char) word_point[i++]);
    if(i % 16 == 0) {
      fprintf(fp, "\n ");
    }
  } 
  fprintf(fp, " 0x%02X",  (unsigned char) word_point[word_length - 1]); 
  fprintf(fp, "\n};\n");
  xfree(word_point);
  fprintf(fp, "%s\n", "int execute_code()");
  fprintf(fp, "%s\n", "{");
  ...
  while(p) {
    ...
    for(i = 0; i < strlen(p->type); i++) {
      type[i] = toupper(p->type[i]);
    }
    fprintf(fp, "  %s *%s = (%s *) xmalloc(%d * sizeof(%s));\n", type, p->type, type, p->max, type);
    p = p->next;
  }
  srand(time(0));
  int r;
  ...
  t = code_node;
  while(t) {
    if(t->flag == 2) fprintf(fp, "  %s\n", t->code);
    t = t->next;
  }
  
  t = code_node;
  while(t) {
    if(t->flag == 4) fprintf(fp, "  %s\n", t->code);
    t = t->next;
  }
